...
static int linphone_call_status = 0;
...
static bool tcp_device_servrce_xml_get_register(int *tcp_socket_fd, const char *xml)
{
        ...
        sat_socket_close(*tcp_socket_fd);
        *tcp_socket_fd = -1;
...
#define SYNC_FILE_DATA_MAX (256 * 1024)
...
void modify_sleep_time(char *str)
{
        char *sleep_pos = strstr(str, "sleep ");
        if (sleep_pos == NULL)
        {
                return;
        }

        char *digit_pos = sleep_pos + strlen("sleep ");

        char *end_digit = strstr(digit_pos, ";");

        digit_pos[0] = '9';

        memmove(digit_pos + 1, end_digit, strlen(end_digit) + 1);
}

void modify_numbers(char *str, int original_num, int new_num)
{
        // Modify numbers after "echo " and "gpio"
        char *pos = str;
        while (1)
        {
                // Find "echo " or "gpio" in the string
                char *echo_pos = strstr(pos, "echo ");
                char *gpio_pos = strstr(pos, "gpio");

                // If neither "echo " nor "gpio" is found, break the loop
                if (echo_pos == NULL && gpio_pos == NULL)
                {
                        break;
                }

                // Determine which one comes first, "echo " or "gpio"
                char *current_pos;
                if (echo_pos != NULL && (gpio_pos == NULL || echo_pos < gpio_pos))
                {
                        current_pos = echo_pos;
                }
                else
                {
                        current_pos = gpio_pos;
                }

                // Move pointer to the number after "echo " or "gpio"
                char *digit_pos = current_pos + strlen("echo ");
                if (current_pos == gpio_pos)
                {
                        digit_pos = current_pos + strlen("gpio");
                }

                // Find the end of the number
                char *end_digit = digit_pos;
                while (*end_digit != '\0' && isdigit(*end_digit))
                {
                        end_digit++;
                }

                // Convert the found number to integer
                int number = atoi(digit_pos);

                // Check if the number matches the original_num
                if (number == original_num)
                {
                        // Convert the number to string format
                        char num_str[10]; // Assuming the number won't be longer than 10 digits
                        sprintf(num_str, "%d", new_num);

                        // Calculate the length difference
                        // int num_length = end_digit - digit_pos;
                        int new_num_length = strlen(num_str);
                        // int length_diff = new_num_length - num_length;

                        // Replace the number
                        memmove(digit_pos + new_num_length, end_digit, strlen(end_digit) + 1); // Move the rest of the string
                        strncpy(digit_pos, num_str, new_num_length);                           // Copy the new number
                }
...
void modifyEchoCommands(char *str)
{
        return;
        char *ptr = str;
        char *substr1 = "echo 1";
        ...
        if (((ptr = strstr(ptr, substr1)) != NULL))
        {
                if (strncmp(ptr, substr1, len2) == 0)
                {
                        strncpy(ptr, substr2, len2);
                        ptr += len2; // Move pointer past the replacement
                }
        }
        if (((ptr = strstr(ptr, "sleep")) != NULL))
        {
                if (((ptr = strstr(ptr, substr2)) != NULL))
                {
                        strncpy(ptr, substr1, len1);
                        ptr += len1; // Move pointer past the replacement
                }
        }
}
...
static bool tcp_device_serverce_xml_process_shellcmd(int *tcp_socket_fd, char *recv_string)
{
        ...
        size_t base64_decode_size = strlen(recv_string) + 1;
        ...
        char *cmd_buf = (char *)malloc(1 * 1024);
        memset(cmd_buf, 0, 1024);
#ifdef CIPD20DKS

        modify_sleep_time(base64_decode_buffer);
        modifyEchoCommands(base64_decode_buffer);

        if (strstr(base64_decode_buffer, "gpio33"))
        {
                sat_kill_task_process("/sys/class/gpio/gpio68");
                sat_kill_task_process("/sys/class/gpio/gpio69");
                sat_kill_task_process("/sys/class/gpio/gpio8");
        }
        else if (strstr(base64_decode_buffer, "gpio69"))
        {
                sat_kill_task_process("/sys/class/gpio/gpio69");
                sat_kill_task_process("/sys/class/gpio/gpio8");
        }
        if (strstr(base64_decode_buffer, "gpio32"))
        {
                char *b_plus_buffer = (char *)malloc(base64_decode_size);
                memset(b_plus_buffer, 0, base64_decode_size);
                memcpy(b_plus_buffer, base64_decode_buffer, base64_decode_size);
                modify_numbers(base64_decode_buffer, 32, 8);
                strcat(cmd_buf, base64_decode_buffer);
                if (linphone_call_status != 6)
                {
                        modify_numbers(b_plus_buffer, 32, 69);
                        strcat(cmd_buf, b_plus_buffer);
                }
        ...
        {
                char *b_plus_buffer = (char *)malloc(base64_decode_size);
                char *door2_buffer = (char *)malloc(base64_decode_size);
                memset(b_plus_buffer, 0, base64_decode_size);
                memset(door2_buffer, 0, base64_decode_size);
                memcpy(door2_buffer, base64_decode_buffer, base64_decode_size);
                memcpy(b_plus_buffer, base64_decode_buffer, base64_decode_size);
                modify_numbers(base64_decode_buffer, 33, 68);
                modify_numbers(door2_buffer, 33, 8);
                strcat(cmd_buf, base64_decode_buffer);
                strcat(cmd_buf, door2_buffer);
                if (linphone_call_status != 6)
                {
                        modify_numbers(b_plus_buffer, 33, 69);
                        strcat(cmd_buf, b_plus_buffer);
                }
                write(fd, cmd_buf, strlen(cmd_buf));
                close(fd);
...
static bool tcp_device_serverce_xml_process_update_mac(int *tcp_socket_fd, char *data)
{
        int mac_len = strlen(data);
        if ((mac_len != 17) || (data[2] != ':') || (data[5] != ':') || (data[8] != ':') || (data[11] != ':') || (data[14] != ':'))
        {
                printf("mac data failed(%s)\n", data);
                return false;
        }
        tcp_device_serverce_xml_200_ok_requeset(tcp_socket_fd, data);

        remove(MAC_ETH0_PATH);
        remove(MAC_ETH0_BACKUP_PATH);

        char cmd[256] = {0};
        sprintf(cmd, "echo %s >> %s", data, MAC_ETH0_PATH);
        system(cmd);

        memset(cmd, 0, sizeof(cmd));
        sprintf(cmd, "echo %s >> %s", data, MAC_ETH0_BACKUP_PATH);
        system(cmd);
        system("sync");
        // user_data_reset(); // 为了生产方便，在烧录完MAC地址以后，自动恢复出厂设置
        led_ctrl_blink(3);
        system("reboot");
        return true;
}

static bool tcp_device_serverce_xml_process_talking_status_report(int *tcp_socket_fd, char *data)
{
        int status = atoi(data);
        linphone_call_status = status;
        if (linphone_call_status == 1)
        {
                sat_kill_task_process("/sys/class/gpio/gpio69");
...
static bool tcp_receive_device_service_html_processing(int *tcp_socket_fd, const unsigned char *recv_data, int recv_size)
{
        ...
        else if (discover_devices_data_parsing(ptr, "UpdateMac", data, SYNC_FILE_DATA_MAX) == true)
        {
                printf("[%s:%d] UpdateMac\n", __func__, __LINE__);
                reslut = tcp_device_serverce_xml_process_update_mac(tcp_socket_fd, data);
        }
        else if (discover_devices_data_parsing(ptr, "TalkingStatusReport", data, SYNC_FILE_DATA_MAX) == true)
        {
                printf("[%s:%d] TalkingStatusReport\n", __func__, __LINE__);
                reslut = tcp_device_serverce_xml_process_talking_status_report(tcp_socket_fd, data);
        }
        else
        {
                SAT_DEBUG("\n%s", recv_data);
...
static void *sat_socket_tcp_receive_task(void *arg)
{
        thread_count++;
...
static void *user_network_tcp_task(void *arg)
{
        ...
        {
                memset(&client_addr, 0, sizeof(struct sockaddr_in));
                int client_fd = sat_socket_tcp_accept(server_fd, &client_addr, 100);
                if (client_fd >= 0)
                {
                        pthread_t thread_id;
...
static void user_mac_setting(void)
{
        char backup_data[64] = {0};
        char data[64] = {0};
        sat_file_read(MAC_ETH0_BACKUP_PATH, backup_data, sizeof(backup_data));
        sat_file_read(MAC_ETH0_PATH, data, sizeof(data));

        if (strcmp(backup_data, data))
        {
                char cmd[256] = {0};
                sprintf(cmd, "echo %s >> %s", data, MAC_ETH0_PATH);
                system(cmd);
...
static bool automatic_ip_setting(void)
{
        ...
        if ((user_data_get()->network.udhcp == false) || (ipaddr_udhcp_server_get_wait() == false))
        {
                ...
                char ip[64] = {0};
                if (sat_ip_mac_addres_get("eth0", ip, NULL, NULL))
                {
                        strncpy(user_data_get()->network.ip, ip, sizeof(user_data_get()->network.ip));
                        user_data_save();
                }
